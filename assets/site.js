...
const hasSiteStylesheet = () =>
  Array.from(document.styleSheets).some((sheet) => {
    return !!sheet.href && /\/assets\/site-|assets\/site\.css/.test(sheet.href);
  });

const waitForSiteStyles = (startedAt = Date.now()) => {
  if (hasSiteStylesheet() || Date.now() - startedAt > 900) {
    requestAnimationFrame(revealSite);
    return;
  }

  requestAnimationFrame(() => waitForSiteStyles(startedAt));
};
